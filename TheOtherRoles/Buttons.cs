    ...
    static class HudManagerStartPatch
    {
        ...
        public static void createButtonsPostfix(HudManager __instance) {
            ...
            sheriffKillButton = new CustomButton(
                () => {
                    MurderAttemptResult murderAttemptResult = TORHelpers.checkMuderAttempt(Sheriff.sheriff, Sheriff.currentTarget);
                    ...
                    if (murderAttemptResult == MurderAttemptResult.PerformKill) {
                        ...
                        if ((Sheriff.currentTarget.Data.Role.IsImpostor && (Sheriff.currentTarget != Mini.mini || Mini.isGrownUp())) ||
                            ...
                            (Sheriff.canKillNeutrals && TORHelpers.isNeutral(Sheriff.currentTarget)) ||
                        ...
                        if (targetId == Sheriff.sheriff.PlayerId && TORHelpers.checkArmored(Sheriff.sheriff, true, true))
                            return;
            ...
            morphlingButton = new CustomButton(
                ...
                () => { return (Morphling.currentTarget || Morphling.sampledTarget) && PlayerControl.LocalPlayer.CanMove && !TORHelpers.MushroomSabotageActive(); },
                () => { 
            ...
            hackerVitalsButton = new CustomButton(
               ...
               () => {
                   ...
                   hackerVitalsButton.actionButton.graphic.sprite = TORHelpers.isMira() ? Hacker.getLogSprite() : Hacker.getVitalsSprite();
                   hackerVitalsButton.actionButton.OverrideText(TORHelpers.isMira() ? "DOORLOG" : "VITALS");
               ...
               () => {
                   ...
                   if (Minigame.Instance) {
                       if (TORHelpers.isMira()) Hacker.doorLog.ForceClose();
                       else Hacker.vitals.ForceClose();
                   }
               },
               false,
              TORHelpers.isMira() ? "DOORLOG" : "VITALS"
           );
            ...
            vampireKillButton = new CustomButton(
                () => {
                    MurderAttemptResult murder = TORHelpers.checkMuderAttempt(Vampire.vampire, Vampire.currentTarget);
                        ...
                        if (Vampire.targetNearGarlic) {
                            ...
                            FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(Vampire.delay, new Action<float>((p) => { // Delayed action
                                ...
                                if (p == 1f) {
                                    ...
                                    var res = TORHelpers.checkMurderAttemptAndKill(Vampire.vampire, Vampire.bitten, showAnimation: false);
            ...
            jackalKillButton = new CustomButton(
                () => {
                    if (TORHelpers.checkMurderAttemptAndKill(Jackal.jackal, Jackal.currentTarget) == MurderAttemptResult.SuppressKill) return;
            ...
            sidekickKillButton = new CustomButton(
                () => {
                    if (TORHelpers.checkMurderAttemptAndKill(Sidekick.sidekick, Sidekick.currentTarget) == MurderAttemptResult.SuppressKill) return;
            ...
            jackalAndSidekickSabotageLightsButton = new CustomButton(
                ...
                () => {
                    if (TORHelpers.sabotageTimer() > jackalAndSidekickSabotageLightsButton.Timer || TORHelpers.sabotageActive())
                        jackalAndSidekickSabotageLightsButton.Timer = TORHelpers.sabotageTimer() + 5f;  // this will give imps time to do another sabotage.
                    return TORHelpers.canUseSabotage();},
                () => {
                    jackalAndSidekickSabotageLightsButton.Timer = TORHelpers.sabotageTimer() + 5f;
                },
            ...
            warlockCurseButton = new CustomButton(
                () => {
                    if (Warlock.curseVictim == null) {
                        ...
                        MurderAttemptResult murder = TORHelpers.checkMurderAttemptAndKill(Warlock.warlock, Warlock.curseVictimTarget, showAnimation: false);
            ...
            securityGuardButton = new CustomButton(
                () => {
                    ...
                    } else if (!TORHelpers.isMira() && !TORHelpers.isFungle() && !SubmergedCompatibility.IsSubmerged) { // Place camera if there's no vent and it's not MiraHQ or Submerged
                        var pos = PlayerControl.LocalPlayer.transform.position;
                        byte[] buff = new byte[sizeof(float) * 2];
                        Buffer.BlockCopy(BitConverter.GetBytes(pos.x), 0, buff, 0*sizeof(float), sizeof(float));
                        Buffer.BlockCopy(BitConverter.GetBytes(pos.y), 0, buff, 1*sizeof(float), sizeof(float));
                ...
                () => {
                    securityGuardButton.actionButton.graphic.sprite = (SecurityGuard.ventTarget == null && !TORHelpers.isMira() && !TORHelpers.isFungle() && !SubmergedCompatibility.IsSubmerged) ? SecurityGuard.getPlaceCameraButtonSprite() : SecurityGuard.getCloseVentButtonSprite(); 
                    ...
                    return !TORHelpers.isMira() && !TORHelpers.isFungle() && !SubmergedCompatibility.IsSubmerged && SecurityGuard.remainingScrews >= SecurityGuard.camPrice && PlayerControl.LocalPlayer.CanMove;
                },
                () => { securityGuardButton.Timer = securityGuardButton.MaxTimer; },
            ...
            securityGuardCamButton = new CustomButton(
                () => {
                    if (!TORHelpers.isMira()) {
                        if (SecurityGuard.minigame == null) {
                            ...
                            if (TORHelpers.isSkeld() || mapId == 3) e = UnityEngine.Object.FindObjectsOfType<SystemConsole>().FirstOrDefault(x => x.gameObject.name.Contains("SurvConsole"));
                            else if (TORHelpers.isAirship()) e = UnityEngine.Object.FindObjectsOfType<SystemConsole>().FirstOrDefault(x => x.gameObject.name.Contains("task_cams"));
                            if (e == null || Camera.main == null) return;
                ...
                () => {
                    ...
                    securityGuardCamButton.actionButton.graphic.sprite = TORHelpers.isMira() ? SecurityGuard.getLogSprite() : SecurityGuard.getCamSprite();
                    securityGuardCamButton.actionButton.OverrideText(TORHelpers.isMira() ? "DOORLOG" : "SECURITY");
                ...
                false,
                TORHelpers.isMira() ? "DOORLOG" : "SECURITY"
            );
            ...
            witchSpellButton = new CustomButton(
                ...
                () => {
                    ...
                    MurderAttemptResult attempt = TORHelpers.checkMuderAttempt(Witch.witch, Witch.spellCastingTarget);
            ...
            ninjaButton = new CustomButton(
                () => {
                    ...
                    if (Ninja.ninjaMarked != null) {
                        ...
                        MurderAttemptResult attempt = TORHelpers.checkMuderAttempt(Ninja.ninja, Ninja.ninjaMarked);
                        if (attempt == MurderAttemptResult.PerformKill) {
            ...
            mayorMeetingButton = new CustomButton(
               () => {
	               ...
	               TORHelpers.handleVampireBiteOnBodyReport(); // Manually call Vampire handling, since the CmdReportDeadBody Prefix won't be called
            ...
            bomberButton = new CustomButton(
                () => {
                    if (TORHelpers.checkMuderAttempt(Bomber.bomber, Bomber.bomber, ignoreMedic: true) != MurderAttemptResult.BlankKill) {
                        var pos = PlayerControl.LocalPlayer.transform.position;
                        byte[] buff = new byte[sizeof(float) * 2];
                        Buffer.BlockCopy(BitConverter.GetBytes(pos.x), 0, buff, 0 * sizeof(float), sizeof(float));
                        Buffer.BlockCopy(BitConverter.GetBytes(pos.y), 0, buff, 1 * sizeof(float), sizeof(float));
            ...
            thiefKillButton = new CustomButton(
                () => {
                    ...
                    var result = TORHelpers.checkMuderAttempt(thief, target);
            ...
            zoomOutButton = new CustomButton(
                () => { TORHelpers.toggleZoom();
                },
                () => { if (PlayerControl.LocalPlayer == null || !PlayerControl.LocalPlayer.Data.IsDead || (PlayerControl.LocalPlayer.Data.Role.IsImpostor && !OptionGroupSingleton<GameplaySettings>.Instance.DeadImpsBlockSabotage)) return false;
                    var (playerCompleted, playerTotal) = TasksHandler.taskInfo(PlayerControl.LocalPlayer.Data);
                    int numberOfLeftTasks = playerTotal - playerCompleted;
                    return numberOfLeftTasks <= 0 || !OptionGroupSingleton<GameplaySettings>.Instance.FinishTasksBeforeHauntingOrZoomingOut;
                },
                () => { return true; },
                () => { return; },
