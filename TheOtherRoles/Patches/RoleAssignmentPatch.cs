    ...
    class LegacyGameOptionsValidatePatch {
        public static void Postfix(LegacyGameOptions __instance) {
            ...
            if (TORMapOptions.GameMode == CustomGamemodes.PropHunt)
                __instance.NumImpostors = Mathf.RoundToInt(OptionGroupSingleton<HunterSettings>.Instance.PropHuntNumberOfHunters);
        ...
        public static void assignRoleTargets(RoleAssignmentData data) {
            ...
            if (Lawyer.lawyer != null) {
                ...
                if (!Lawyer.isProsecutor) { // Lawyer
                    ...
                    foreach (PlayerControl p in PlayerControl.AllPlayerControls) {
                        if (!p.Data.IsDead && !p.Data.Disconnected && p != Lovers.lover1 && p != Lovers.lover2 && p != Mini.mini && !p.Data.Role.IsImpostor && !TORHelpers.isNeutral(p) && p != Swapper.swapper)
                            possibleTargets.Add(p);
                    }
                }
