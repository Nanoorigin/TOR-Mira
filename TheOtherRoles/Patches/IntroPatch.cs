...
namespace TheOtherRoles.Patches {
                   ...
                   // PlayerControl.SetPetImage(data.DefaultOutfit.PetId, data.DefaultOutfit.ColorId, player.PetSlot);
                    ...
                    if (PlayerControl.LocalPlayer == Arsonist.arsonist && p != Arsonist.arsonist) {
                        ...
                        if (HideNSeek.isHunted() && p.Data.Role.IsImpostor) {
                            ...
                            player.cosmetics.nameText.text += $"{TORHelpers.cs(Color.red, " (Hunter)")}";
                            player.gameObject.SetActive(true);
    ...
    class IntroPatch {
        public static void setupIntroTeamIcons(IntroCutscene __instance, ref  Il2CppSystem.Collections.Generic.List<PlayerControl> yourTeam) {
            // Intro solo teams
            if (TORHelpers.isNeutral(PlayerControl.LocalPlayer)) {
        ...
        public static void setupIntroTeam(IntroCutscene __instance, ref  Il2CppSystem.Collections.Generic.List<PlayerControl> yourTeam) {
            ...
            if (roleInfo == null || roleInfo == RoleInfo.crewmate) {
                if (RoleDraft.isEnabled && CustomOptionHolder.neutralRolesCountMax.getSelection() > 0) {
                    __instance.TeamTitle.text = "<size=60%>Crewmate" + TORHelpers.cs(Color.white, " / ") + TORHelpers.cs(neutralColor, "Neutral") + "</size>";
                }
                return;
            }
