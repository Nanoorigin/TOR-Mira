        ...
        public static void Postfix(EndGameManager __instance) {
            ...
            for (int i = 0; i < list.Count; i++) {
                ...
                foreach(var data in AdditionalTempData.playerRoles) {
                    ...
                    var roles = 
                    poolablePlayer.cosmetics.nameText.text += $"\n{string.Join("\n", data.Roles.Select(x => TORHelpers.cs(x.color, x.name)))}";
                }
            }
            ...
            foreach (WinCondition cond in AdditionalTempData.additionalWinConditions) {
                if (cond == WinCondition.AdditionalLawyerBonusWin) {
                    textRenderer.text += $"\n{TORHelpers.cs(Lawyer.color, "The Lawyer wins with the client")}";
                } else if (cond == WinCondition.AdditionalAlivePursuerWin) {
                    textRenderer.text += $"\n{TORHelpers.cs(Pursuer.color, "The Pursuer survived")}";
                }
            }
            ...
            if (TORMapOptions.showRoleSummary || HideNSeek.isHideNSeekGM || PropHunt.isPropHuntGM) {
                ...
                foreach(var data in AdditionalTempData.playerRoles) {
                    ...
                    roleSummaryText.AppendLine($"{TORHelpers.cs(data.IsAlive ? Color.white : new Color(.7f,.7f,.7f), data.PlayerName)} - {roles}{taskInfo}"); 
                }
                ...
                TORHelpers.previousEndGameSummary = $"<size=110%>{roleSummaryText.ToString()}</size>";
            }
            AdditionalTempData.clear();
        }
    }
