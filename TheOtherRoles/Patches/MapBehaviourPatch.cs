...
namespace TheOtherRoles.Patches {
	...
	static class MapBehaviourPatch {
		...
		public static Sprite Vent = TORHelpers.loadSpriteFromResources("TheOtherRoles.Resources.Vent.png", 150f);
		...
		static void Postfix(MapBehaviour __instance) {
			if (__instance.infectedOverlay.gameObject.active && PlayerControl.LocalPlayer.Data.IsDead && OptionGroupSingleton<GameplaySettings>.Instance.DeadImpsBlockSabotage) {
				__instance.Close();
			}
				...
				if (numberOfTasks == 0) {
					if (MeetingHud.Instance == null) {
						foreach (PlayerControl player in PlayerControl.AllPlayerControls) {
							if (Snitch.targets == Snitch.Targets.EvilPlayers && !TORHelpers.isEvil(player)) continue;
							else if (Snitch.targets == Snitch.Targets.Killers && !TORHelpers.isKiller(player)) continue;
			...
			if (PlayerControl.LocalPlayer.Data.IsDead && (!PlayerControl.LocalPlayer.Data.Role.IsImpostor || OptionGroupSingleton<GameplaySettings>.Instance.DeadImpsBlockSabotage)) {
				foreach (PlayerControl player in PlayerControl.AllPlayerControls) {
