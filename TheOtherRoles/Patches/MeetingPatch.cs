    ...
    class MeetingHudPatch {
            ...
            private static Dictionary<byte, int> CalculateVotes(MeetingHud __instance) {
                ...
                for (int i = 0; i < __instance.playerStates.Length; i++) {
                    ...
                    if (playerVoteArea.VotedFor != 252 && playerVoteArea.VotedFor != 255 && playerVoteArea.VotedFor != 254) {
                        PlayerControl player = TORHelpers.PlayerById((byte)playerVoteArea.TargetPlayerId);
        ...
        static void swapperOnClick(int i, MeetingHud __instance) {
            ...
            if (selectedCount == 0) {
                ...
                if (selections[i]) {
                    ...
                    selections[i] = true;
                    renderer.color = Color.yellow;
                    meetingExtraButtonLabel.text = TORHelpers.cs(Color.yellow, "Confirm Swap");
                }
            } else if (selectedCount == 2) {
                if (selections[i]) {
                    renderer.color = Color.red;
                    selections[i] = false;
                    meetingExtraButtonLabel.text = TORHelpers.cs(Color.red, "Confirm Swap");
                }
            }
        }
        ...
        static void swapperConfirm(MeetingHud __instance) {
            ...
            if (firstPlayer != null && secondPlayer != null) {
                ...
                meetingExtraButtonLabel.text = TORHelpers.cs(Color.green, "Swapping!");
                Swapper.charges--;
        ...
        static void mayorToggleVoteTwice(MeetingHud __instance) {
            ...
            meetingExtraButtonLabel.text = TORHelpers.cs(Mayor.color, "Double Vote: " + (Mayor.voteTwice ? TORHelpers.cs(Color.green, "On ") : TORHelpers.cs(Color.red, "Off")));
        }
        ...
        static void guesserOnClick(int buttonTarget, MeetingHud __instance) {
            ...
            foreach (RoleInfo roleInfo in RoleInfo.allRoleInfos) {
                ...
                label.text = TORHelpers.cs(roleInfo.color, roleInfo.name);
                ...
                if (!PlayerControl.LocalPlayer.Data.IsDead && !TORHelpers.PlayerById((byte)__instance.playerStates[buttonTarget].TargetPlayerId).Data.IsDead) button.GetComponent<PassiveButton>().OnClick.AddListener((System.Action)(() => {
                    if (selectedButton != button) {
                        selectedButton = button;
                        buttons.ForEach(x => x.GetComponent<SpriteRenderer>().color = x == selectedButton ? Color.red : Color.white);
                    } else {
                        PlayerControl focusedTarget = TORHelpers.PlayerById((byte)__instance.playerStates[buttonTarget].TargetPlayerId);
        ...
        static void populateButtonsPostfix(MeetingHud __instance) {
            ...
            if (isGuesser && !PlayerControl.LocalPlayer.Data.IsDead && remainingShots > 0) {
                for (int i = 0; i < __instance.playerStates.Length; i++) {
                    ...
                    if (PlayerControl.LocalPlayer != null && !TORHelpers.isEvil(PlayerControl.LocalPlayer) && playerCompleted < HandleGuesser.tasksToUnlock) continue;
        ...
        class StartMeetingPatch {
            public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)]NetworkedPlayerInfo meetingTarget) {
                ...
                if (Portalmaker.portalmaker != null && (PlayerControl.LocalPlayer == Portalmaker.portalmaker || TORHelpers.shouldShowGhostInfo()) && !Portalmaker.portalmaker.Data.IsDead) {
                ...
                if (Trapper.trapper != null && (PlayerControl.LocalPlayer == Trapper.trapper || TORHelpers.shouldShowGhostInfo()) && !Trapper.trapper.Data.IsDead) {
                    ...
                    foreach (Trap trap in Trap.traps) {
                        ...
                        foreach (byte playerId in trap.trappedPlayer) {
                            PlayerControl p = TORHelpers.PlayerById(playerId);
                            if (Trapper.infoType == 0) message += RoleInfo.GetRolesString(p, false, false, true) + "\n";
                            else if (Trapper.infoType == 1) {
                                if (TORHelpers.isNeutral(p) || p.Data.Role.IsImpostor) message += "Evil Role \n";
                ...
                if (Snitch.snitch != null && Snitch.mode != Snitch.Mode.Map && (PlayerControl.LocalPlayer == Snitch.snitch || TORHelpers.shouldShowGhostInfo()) && !Snitch.snitch.Data.IsDead) {
                    var (playerCompleted, playerTotal) = TasksHandler.taskInfo(Snitch.snitch.Data);
                        ...
                        FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(0.4f, new Action<float>((x) => {
                            if (x == 1f) {
                                foreach (PlayerControl p in PlayerControl.AllPlayerControls) {
                                    if (Snitch.targets == Snitch.Targets.Killers && !TORHelpers.isKiller(p)) continue;
                                    if (Snitch.targets == Snitch.Targets.EvilPlayers && !TORHelpers.isEvil(p)) continue;
